...
function useCardsState(colorDisposition) {
  ...
  const [cardsState, setCardsState] = useState(
    colorDisposition.map(color => ({ visible: false, color, present: true }))
  );
  ...
  useEffect(() => {
    ...
    if (firstFoundIdx !== -1) {
      const secondFoundIdx = checkTurnedCard(
        cardsState.slice(firstFoundIdx + 1, cardsState.length)
      );
      ...
      if (secondFoundIdx !== -1) {
        ...
        timerRef.current = setTimeout(() => {
          ...
          if (firstCardFlipped.color === secondCardFlipped.color) {
            ...
            newState[firstFlippedCardIdx] = {
              ...firstCardFlipped,
              visible: false,
              present: false,
            };
            newState[secondFlippedCardIdx] = {
              ...secondCardFlipped,
              visible: false,
              present: false,
            };
